...
BN_SNIPE_MIN_ABS_RATE = 0.002           # 最小绝对费率 0.20%（覆盖双边手续费+最小滑点，低于此必亏）
BN_SNIPE_POSITION_SIZE_RATIO = 0.98     # 合约余额使用率（裸仓单腿，留 2% 缓冲）
BN_SNIPE_OPEN_OFFSET_MS = 1000           # 开仓相对结算点偏移：正=结算前N ms，负=结算后N ms。1000=结算前1s开仓（RANK_TIMING关闭时的固定值）

# ── 按币安字母排位线性设时点 ──
# rank% = 该币 base 在全 USDT 永续（含TradFi股票币）字母序中的百分位（0=最靠前 → ~1=最靠后）。
# 依据：币安结算按符号顺序批处理，字母越靠后资费记账越晚（实测 rank% vs 记账偏移 Pearson≈0.77）。
# 开仓延迟、平仓死线都随 rank% 在 [MIN, MAX] 间线性插值：靠前→早开早平省暴露，靠后→晚开晚平保资费。
BN_SNIPE_RANK_TIMING_ENABLED = True      # True=按字母排位线性设开/平时点；False=用上方固定 OPEN_OFFSET + 400ms
BN_SNIPE_OPEN_MS_MIN  = 200.0            # rank%=0(最靠前) 的开仓延迟（结算后ms，T实测≥264ms，留64ms安全垫）
BN_SNIPE_OPEN_MS_MAX  = 250.0            # rank%=1(最靠后) 的开仓延迟
BN_SNIPE_CLOSE_MS_MIN = 360.0            # rank%=0 的平仓硬死线（小批次T≤342ms，留18ms余量）
BN_SNIPE_CLOSE_MS_MAX = 400.0            # rank%=1 的平仓硬死线（大批次T≤374ms，留26ms余量）
BN_SNIPE_TRADFI_OPEN_MS = 80.0           # TradFi股票合约开仓固定延迟（T实测~107ms，早于普通币264ms）

BN_SNIPE_TAKE_PROFIT_ENABLED = False    # [测试] 关闭止盈，防止5s持有期内提前触发平仓干扰资费采集
