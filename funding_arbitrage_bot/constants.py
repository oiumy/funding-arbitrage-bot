...
BN_SNIPE_POSITION_SIZE_RATIO = 0.95     # 合约余额使用率（裸仓单腿，留 5% 缓冲，防保证金不足）
BN_SNIPE_OPEN_OFFSET_MS = -120           # 开仓相对结算点偏移：正=结算前N ms，负=结算后N ms。-120=结算后120ms开仓（RANK_TIMING关闭时的固定值）
...
BN_SNIPE_OPEN_MS_MIN  = 120.0            # rank%=0(最靠前) 的开仓延迟（结算后ms）
BN_SNIPE_OPEN_MS_MAX  = 120.0            # rank%=1(最靠后) 的开仓延迟；=MIN → 所有币固定120ms
BN_SNIPE_CLOSE_MS_MIN = 340.0            # rank%=0 的平仓硬死线
BN_SNIPE_CLOSE_MS_MAX = 340.0            # rank%=1 的平仓硬死线；=MIN → 所有币固定340ms
BN_SNIPE_TRADFI_OPEN_MS = 50.0           # TradFi股票合约开仓固定延迟（80ms仍踩空资费，实测其T可能<83ms，再前移）
