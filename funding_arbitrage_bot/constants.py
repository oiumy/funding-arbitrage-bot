...
BN_SNIPE_OPEN_OFFSET_MS = -100          # 开仓相对结算点偏移：正=结算前N ms，负=结算后N ms。-100=结算后100ms开仓（RANK_TIMING关闭时的固定值）

# ── 按币安字母排位线性设时点 ──
# rank% = 该币 base 在全 USDT 永续(排除TradFi)字母序中的百分位（0=最靠前 → ~1=最靠后）。
...
BN_SNIPE_OPEN_MS_MIN  = 100.0            # rank%=0(最靠前) 的开仓延迟（结算后ms）
BN_SNIPE_OPEN_MS_MAX  = 100.0            # rank%=1(最靠后) 的开仓延迟；=MIN → 开仓恒定100ms（必须早于最早快照~120ms，不随排位冒险）
BN_SNIPE_CLOSE_MS_MIN = 200.0            # rank%=0 的平仓硬死线（结算后ms）
BN_SNIPE_CLOSE_MS_MAX = 480.0            # rank%=1 的平仓硬死线（抬到480以盖住尾部晚记账币，如T~417ms）
